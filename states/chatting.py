...
async def quit_conversation(message: Message, bot: Bot):
    ...
    await message.answer("You've left the conversation", reply_markup=only_back.as_markup())
    companion_id = await get_chatting_with_id(user_id=user_id)
    await update_is_searching(user_id, 0, None)
    if companion_id:
        await bot.send_message(chat_id=companion_id, text="Your companion has decided to leave the conversation.",
...
async def handle_chat(message: Message, bot: Bot):
    ...
    if companion_id:
        ...
        await bot.send_message(chat_id=companion_id, text=text_plus_translation, parse_mode="HTML", reply_markup=skip_companion.as_markup(resize_keyboard=True))
